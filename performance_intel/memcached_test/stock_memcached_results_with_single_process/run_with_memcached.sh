...
ssh $user@$seastar_server_name "memcached -t $vcpu -m 24000 > /tmp/null &"
...
for ((i = 0; i < $clients; ++i)); do port=$((11211)); taskset -c $(($i*2)),$(($i*2+1)) memaslap -s $seastar_ip:$port -t 60s -T 2 -c 60 -X 64 > $out.$i & done
wait `pgrep memaslap`
